...
class UpdateClassificationRuleRequest extends FormRequest
{
    ...
    {
        /** @var ClassificationRule|null $classificationRule */
        ...
        return [
            'chave' => [
                ...
                'string',
                'max:255',
                function (string $attribute, mixed $value, \Closure $fail) use ($classificationRule): void {
                    if (! is_string($value)) {
                        return;
                    }
                    ...
                    $exists = ClassificationRule::query()
                        ...
                        ->when(
                            $classificationRule instanceof ClassificationRule,
                            fn ($query) => $query->whereKeyNot($classificationRule->getKey()),
                        )
                        ->exists();
